...
export function ensureSchema(env) {
  if (!env.DB) {
    const error = new Error('Databasebinding DB ontbreekt.');
    error.status = 503;
    throw error;
  }
  if (!schemaPromise) {
...
export async function schemaReady(env) {
  ...
  try {
    ...
    const row = await env.DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='core_stories' LIMIT 1").first();
    return Boolean(row);
  } catch {
    return false;
  }
