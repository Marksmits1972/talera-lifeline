...
const ORB_V79_STYLE = String.raw`
/* TALERA ORB v79 — v78 preserved; only internal blue contrast is raised subtly. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb79Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V79_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function makeLayer(seed,kind){
    ...
    for(var py=0;py<TEX;py++){
      ...
      for(var px=0;px<TEX;px++){
        ...
        /* V79: subtle 8% contrast lift around the existing palette midpoint.
           Geometry, alpha, motion and edge treatment remain unchanged from v78. */
        var contrast=1.08;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v79')==='1')return;
    core.setAttribute('data-orb-v79','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v79-canvas';
    ...
    function frame(now){
      ...
      /* Voice response remains exactly v78. */
      ...
      /* Whole-body scale response remains exactly v78. */
      ...
      /* Preserve the v78 soft light-blue perimeter exactly. */
      ctx.save();
      var rimWidth=Math.max(1.5,w*.006);
      ctx.beginPath();
      ctx.arc(cx,cy,r-rimWidth*.72,0,Math.PI*2);
      ctx.strokeStyle='rgba(132,190,232,.46)';
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V79_STYLE+'</style></head>')
    .replace('</body>',ORB_V79_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v79-subtle-internal-contrast');
