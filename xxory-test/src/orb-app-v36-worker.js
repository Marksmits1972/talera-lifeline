...
const ORB_V36_STYLE = String.raw`
/* TALERA ORB v36 — v35 macro scale with a medium cloud-detail layer and cleaner light breaks. Directly on worker.js. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb36Pulse 5.35s ease-in-out infinite!important;
}
...
.status::after{content:"  · v36";font-size:9px;opacity:.34;vertical-align:middle}

@keyframes taleraOrb36Pulse{
...
const ORB_V36_SCRIPT = String.raw`<script>(function(){
  var LOW=116;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v36')==='1')return;
    core.setAttribute('data-orb-v36','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v36-canvas';
    ...
    function render(now){
      ...
      for(var py=0;py<LOW;py++){
        ...
        for(var px=0;px<LOW;px++){
          ...
          var edgeBias=mid*.105;
          var softCloud=smoothstep(.415,.570,density+edgeBias*.52);
          var cloud=smoothstep(.482,.622,density+edgeBias);

          var deep=fbm(wx*.66+t*.005,wy*.63-t*.0045,89);
          var deepMedium=fbm(wx*.98-t*.0035,wy*.91+t*.0028,113);
          var deepCloud=smoothstep(.49,.68,deep*.78+deepMedium*.22);
          ...
          var lamp1=gauss(nx,ny,c1x,c1y,.245,.195);
          var lamp2=gauss(nx,ny,c2x,c2y,.155,.128)*.50;
          var opening1=smoothstep(.15,.72,clear1)*(1-cloud*.84);
          var opening2=smoothstep(.18,.70,clear2)*(1-cloud*.88);
          var lamp=Math.max(lamp1*opening1,lamp2*opening2);
          lamp*=.58+.09*Math.sin(t*.66)+awake*.08+voice*.11;

          var white=softCloud*.32 + cloud*(.53+.10*n2);
          var shade=deepCloud*(1-softCloud)*.24 + cloud*deepCloud*.14;

          var rim1=smoothstep(.16,.58,clear1)*softCloud*(1-cloud);
          var rim2=smoothstep(.18,.55,clear2)*softCloud*(1-cloud)*.65;
          var rim=(rim1+rim2)*.22;

          var R=br, G=bg, B=bb;
          R=R*(1-shade)+47*shade;
          G=G*(1-shade)+99*shade;
          B=B*(1-shade)+136*shade;
      ...
      g.addColorStop(.45,'rgba(226,244,252,.018)');
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V36_STYLE+'</style></head>')
    .replace('</body>',ORB_V36_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v36-canvas-medium-detail-clouds');
