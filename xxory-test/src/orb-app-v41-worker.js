...
const ORB_V41_STYLE = String.raw`
/* TALERA ORB v41 — v40 cloud masses with edge-only fractal breakup. Directly on worker.js. */
...
.orb-v41-canvas{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;display:block!important;pointer-events:none!important}
.status::after{content:"  · v41";font-size:9px;opacity:.34;vertical-align:middle}
@keyframes taleraOrb41Pulse{0%,16%,100%{scale:.982;border-radius:49% 51% 48% 52% / 52% 47% 53% 48%}38%{scale:1.017;border-radius:51% 49% 52% 48% / 48% 53% 47% 52%}50%{scale:1.035}62%{scale:1.002}73%{scale:1.020}}
`;

const ORB_V41_SCRIPT = String.raw`<script>(function(){
  var LOW=142;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v41')==='1')return;
    core.setAttribute('data-orb-v41','1');
    core.innerHTML='';
    var canvas=document.createElement('canvas');canvas.className='orb-v41-canvas';canvas.setAttribute('aria-hidden','true');core.appendChild(canvas);
    ...
    function render(now){
      ...
      for(var py=0;py<LOW;py++){
        ...
        for(var px=0;px<LOW;px++){
          ...
          /* Keep v40's two large cloud decks and union. */
          ...
          /* Clearings stay broad and organic. */
          ...
          /* v41: edge-only fractal breakup.
             Higher-frequency noise affects ONLY the transition band around the cloud threshold.
             The interiors of the large masses stay calm and connected. */
          ...
          var edgeDensity=baseDensity+edgeNoise*.205*boundaryBand;

          /* Slightly narrower cloud thresholds make the ragged contour readable,
             without turning the whole body into grain. */
          var softCloud=smoothstep(.455,.555,edgeDensity);
          var cloud=smoothstep(.505,.590,edgeDensity);

          /* Retain a broad shoulder from the underlying deck so cloud masses stay joined. */
          ...
          /* Light still exists only in true openings. */
          ...
          /* Edge texture brightens only where the cloud is transitioning,
             which should read as a broken cloud rim rather than a white blob. */
          var edgeTexture=(edgeNoise*.5+.5)*boundaryBand*softCloud*(1-cloud*.20);
          var white=softCloud*.30+cloud*(.42+.06*aBody)+edgeTexture*.11;
      ...
      g.addColorStop(.45,'rgba(226,244,252,.007)');
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V41_STYLE+'</style></head>')
    .replace('</body>',ORB_V41_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v41-canvas-edge-fractal-clouds');
