...
const ORB_V59_STYLE = String.raw`
/* TALERA ORB v59 — stronger blue bands, calm larger heart, 6x voice motion. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  box-shadow:
    0 10px 32px rgba(21,60,98,.05),
    0 0 30px rgba(67,145,201,.17),
    0 0 76px rgba(67,145,201,.075)!important;
  filter:none!important;
  transform:scale(calc(.995 + var(--awake)*.075 + var(--orb-life)))!important;
  transition:transform .18s ease-out!important;
  animation:taleraOrb59Body 9.2s ease-in-out infinite!important;
}
...
const ORB_V59_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function makeLayer(seed,kind){
    ...
    for(var py=0;py<TEX;py++){
      ...
      for(var px=0;px<TEX;px++){
        ...
        var alphaBase=kind===0?.62:(kind===1?.72:.50);
        var alpha=(body*.68+soft*.23+filament*(kind===2?.30:.08))*alphaBase;
        alpha*=1-opening*.90;
        alpha*=.79+dense*.21;
        if(alpha<.020)alpha=0;

        /* Three stronger blue zones: deep -> mid -> ice. */
        var field=macro*.58+mid*.42;
        var midGate=smoothstep(.435,.515,field);
        var lightGate=smoothstep(.570,.655,field);

        var deepR=kind===0?16:(kind===1?24:30);
        var deepG=kind===0?58:(kind===1?78:88);
        var deepB=kind===0?112:(kind===1?142:156);

        var midR=kind===0?58:(kind===1?72:82);
        var midG=kind===0?132:(kind===1?154:164);
        var midB=kind===0?194:(kind===1?210:220);

        var iceR=kind===0?198:(kind===1?216:228);
        var iceG=kind===0?229:(kind===1?241:247);
        var iceB=kind===0?247:(kind===1?251:253);
        ...
        /* Strong local valleys, but only inside tissue. */
        var depth=fbm(wx*1.02+1.8,wy*.95-2.3,seed+107);
        var shadow=smoothstep(.45,.68,depth)*(body*.72+soft*.20);
        R=mix(R,12,shadow*.56);
        G=mix(G,42,shadow*.56);
        B=mix(B,84,shadow*.56);

        /* Local pale ridges instead of broad white haze. */
        var glint=smoothstep(.61,.80,macro)*(1-shadow)*(.10+edge*.15);
        R=mix(R,225,glint);G=mix(G,244,glint);B=mix(B,252,glint);

        var i=(py*TEX+px)*4;
        d[i]=R;d[i+1]=G;d[i+2]=B;d[i+3]=Math.round(clamp(alpha,0,.86)*255);
      }
    }
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v59')==='1')return;
    core.setAttribute('data-orb-v59','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v59-canvas';
    ...
    function frame(now){
      ...
      /* Voice controls life energy: fast rise, calm decay. */
      ...
      /* True 6x range: idle .12 -> full speech .72. */
      var idleSpeed=.12;
      var speed=idleSpeed*(1+energy*5.0);
      flowClock+=dt*speed;

      /* Calm fixed heart: one slow inhale/exhale wave, independent of speech. */
      ...
      /* Larger warm heart, still behind all tissue. */
      ...
      ctx.strokeStyle='rgba(44,108,164,.09)';ctx.lineWidth=Math.max(1,w*.0038);ctx.stroke();ctx.restore();
    }
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V59_STYLE+'</style></head>')
    .replace('</body>',ORB_V59_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v59-blue-bands-calm-heart-6x-voice-motion');
