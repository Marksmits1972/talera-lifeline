...
const ORB_V62_STYLE = String.raw`
/* TALERA ORB v62 — freeze v61 design, add whole-body breathing: ~10% idle, ~30% speaking. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb62Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V62_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v62')==='1')return;
    core.setAttribute('data-orb-v62','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v62-canvas';
    ...
    function frame(now){
      ...
      /* Whole ORB breathing is independent from the internal texture clock.
         Idle: about 10% peak-to-peak. Full speech: about 30% peak-to-peak. */
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V62_STYLE+'</style></head>')
    .replace('</body>',ORB_V62_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v62-whole-body-breath-10-idle-30-voice');
