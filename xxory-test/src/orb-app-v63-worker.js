...
const ORB_V63_STYLE = String.raw`
/* TALERA ORB v63 — v62 frozen, heart becomes one uniform TALERA peach hue with no white center. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb63Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V63_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v63')==='1')return;
    core.setAttribute('data-orb-v63','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v63-canvas';
    ...
    function frame(now){
      ...
      /* Whole ORB breathing from v62 stays untouched. */
      var bodyPeriod=5.6-(energy*.8);
      var bodyPhase=((now/1000)%bodyPeriod)/bodyPeriod;
      var bodyWave=Math.sin(bodyPhase*Math.PI*2);
      var bodyAmplitude=.05+energy*.10;
      var bodyCenter=1+energy*.02;
      var bodyScale=bodyCenter+bodyWave*bodyAmplitude;
      core.style.setProperty('--orb-scale',bodyScale.toFixed(4));

      /* Heart rhythm stays untouched; only the color rendering changes. */
      ...
      /* V63 heart: one uniform TALERA hue. No white center/highlight.
         The core and glow share exactly the same RGB; only alpha and radius breathe. */
      var hr=r*(.35+beat*.075);
      var coreRadius=hr*(.34+beat*.025);
      var coreAlpha=.34+heartLevel*.28;

      var hg=ctx.createRadialGradient(hx,hy,coreRadius*.25,hx,hy,hr);
      hg.addColorStop(0,'rgba('+HEART_R+','+HEART_G+','+HEART_B+','+(.22+heartLevel*.22)+')');
      hg.addColorStop(.34,'rgba('+HEART_R+','+HEART_G+','+HEART_B+','+(.20+heartLevel*.20)+')');
      hg.addColorStop(.62,'rgba('+HEART_R+','+HEART_G+','+HEART_B+','+(.075+heartLevel*.11)+')');
      hg.addColorStop(.82,'rgba('+HEART_R+','+HEART_G+','+HEART_B+','+(.025+heartLevel*.040)+')');
      ...
      ctx.fillStyle='rgba('+HEART_R+','+HEART_G+','+HEART_B+','+coreAlpha+')';
      ctx.fill();
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V63_STYLE+'</style></head>')
    .replace('</body>',ORB_V63_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v63-uniform-talera-peach-heart-no-white-core');
