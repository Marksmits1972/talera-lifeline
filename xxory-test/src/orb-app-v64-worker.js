...
const ORB_V64_STYLE = String.raw`
/* TALERA ORB v64 — v63 frozen, heart becomes an organic diffuse glow instead of a circle. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb64Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V64_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v64')==='1')return;
    core.setAttribute('data-orb-v64','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v64-canvas';
    ...
    function frame(now){
      ...
      /* V64 heart: no geometric core. Several overlapping soft lobes in the same
         TALERA peach hue create one irregular warm living glow behind the tissue. */
      var hs=.90+beat*.24;
      var baseAlpha=.12+heartLevel*.10;
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V64_STYLE+'</style></head>')
    .replace('</body>',ORB_V64_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v64-diffuse-irregular-talera-heart-glow');
