...
const ORB_V65_STYLE = String.raw`
/* TALERA ORB v65 — v64 frozen, heart uses one continuous inhale/exhale cycle independent from the organism. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb65Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V65_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v65')==='1')return;
    core.setAttribute('data-orb-v65','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v65-canvas';
    ...
    function frame(now){
      ...
      /* Whole ORB motion remains exactly voice-reactive and independent from the heart. */
      ...
      /* V65 heart: one continuous, even inhale/exhale cycle. No lub-dub, no pause,
         no dependency on voice. One full cycle takes 4.8 seconds. */
      ...
      /* Organic diffuse heart from v64, now breathing smoothly and continuously. */
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V65_STYLE+'</style></head>')
    .replace('</body>',ORB_V65_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v65-continuous-independent-heart-breath');
