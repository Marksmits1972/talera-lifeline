...
const ORB_V66_STYLE = String.raw`
/* TALERA ORB v66 — v65 visual baseline preserved; only voice-reactive motion is calmed. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb66Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V66_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v66')==='1')return;
    core.setAttribute('data-orb-v66','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v66-canvas';
    ...
    function frame(now){
      ...
      /* V66: voice response deliberately damped. V65 was over-reactive on real speech. */
      var voiceTarget=clamp(voiceRaw*1.15,0,.72);
      if(listening&&voiceTarget<.025)voiceTarget=.025;
      var response=voiceTarget>energy?5.2:1.45;
      energy+=(voiceTarget-energy)*Math.min(1,dt*response);

      var idleSpeed=.21;
      var speed=idleSpeed*(1+energy*2.0);
      flowClock+=dt*speed;

      /* Whole ORB still reacts to speech, but no longer explodes in scale. */
      var bodyPeriod=5.6-(energy*.25);
      var bodyPhase=((now/1000)%bodyPeriod)/bodyPeriod;
      var bodyWave=Math.sin(bodyPhase*Math.PI*2);
      var bodyAmplitude=.05+energy*.035;
      var bodyCenter=1+energy*.005;
      var bodyScale=bodyCenter+bodyWave*bodyAmplitude;
      core.style.setProperty('--orb-scale',bodyScale.toFixed(4));

      /* Heart remains exactly independent: one continuous inhale/exhale cycle. */
      ...
      var amp=1.00+energy*.30,s=r*2.49;
      var breathe1=.94+Math.sin(flowClock*.48)*(.046+energy*.008);
      var breathe2=.97+Math.sin(flowClock*.61+1.8)*(.041+energy*.010);

      drawLayer(back,cx+Math.sin(flowClock*.57)*r*.145*amp,cy+Math.cos(flowClock*.47)*r*.122*amp,
        s*1.08,Math.sin(flowClock*.31)*(.15+energy*.035),1.0,breathe1,1.04/breathe1);
      drawLayer(front,cx+Math.sin(flowClock*.84+2.1)*r*.185*amp,cy+Math.cos(flowClock*.68+1.2)*r*.158*amp,
        s*.99,-.10+Math.sin(flowClock*.42+1.2)*(.20+energy*.045),1.0,breathe2,1.035/breathe2);
      drawLayer(filaments,cx+Math.sin(flowClock*.73+3.7)*r*.148*amp,cy+Math.cos(flowClock*.88+2.5)*r*.140*amp,
        s*1.02,.13+Math.sin(flowClock*.53+3.1)*(.23+energy*.045),.82,1.02,.98);
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V66_STYLE+'</style></head>')
    .replace('</body>',ORB_V66_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v66-calmed-voice-response');
