...
const ORB_V67_STYLE = String.raw`
/* TALERA ORB v67 — v65 visual baseline preserved; voice response balanced between v65 and v66. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb67Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V67_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v67')==='1')return;
    core.setAttribute('data-orb-v67','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v67-canvas';
    ...
    function frame(now){
      ...
      /* V67: low and medium speech are clearly visible again, while loud peaks are compressed. */
      ...
      var idleSpeed=.21;
      var speed=idleSpeed*(1+energy*3.4);
      flowClock+=dt*speed;

      /* Idle breathing stays at ~10% peak-to-peak. Speech raises whole-body life
         toward ~20% peak-to-peak instead of the ~30% extreme of v65. */
      var bodyPeriod=5.6-(energy*.45);
      var bodyPhase=((now/1000)%bodyPeriod)/bodyPeriod;
      var bodyWave=Math.sin(bodyPhase*Math.PI*2);
      var bodyAmplitude=.05+energy*.055;
      var bodyCenter=1+energy*.010;
      ...
      var amp=1.00+energy*.55,s=r*2.49;
      var breathe1=.94+Math.sin(flowClock*.48)*(.046+energy*.012);
      var breathe2=.97+Math.sin(flowClock*.61+1.8)*(.041+energy*.014);

      drawLayer(back,cx+Math.sin(flowClock*.57)*r*.145*amp,cy+Math.cos(flowClock*.47)*r*.122*amp,
        s*1.08,Math.sin(flowClock*.31)*(.15+energy*.065),1.0,breathe1,1.04/breathe1);
      drawLayer(front,cx+Math.sin(flowClock*.84+2.1)*r*.185*amp,cy+Math.cos(flowClock*.68+1.2)*r*.158*amp,
        s*.99,-.10+Math.sin(flowClock*.42+1.2)*(.20+energy*.080),1.0,breathe2,1.035/breathe2);
      drawLayer(filaments,cx+Math.sin(flowClock*.73+3.7)*r*.148*amp,cy+Math.cos(flowClock*.88+2.5)*r*.140*amp,
        s*1.02,.13+Math.sin(flowClock*.53+3.1)*(.23+energy*.080),.82,1.02,.98);
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V67_STYLE+'</style></head>')
    .replace('</body>',ORB_V67_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v67-balanced-voice-response');
