...
const ORB_V68_STYLE = String.raw`
/* TALERA ORB v68 — v67 preserved; visible voice response increased about 15%. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb68Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V68_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v68')==='1')return;
    core.setAttribute('data-orb-v68','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v68-canvas';
    ...
    function frame(now){
      ...
      /* Same voice detection as v67: only the visible response below is slightly stronger. */
      ...
      var idleSpeed=.21;
      var speed=idleSpeed*(1+energy*3.9);
      flowClock+=dt*speed;

      /* Idle breathing unchanged. Speech response is ~15% stronger than v67. */
      var bodyPeriod=5.6-(energy*.50);
      var bodyPhase=((now/1000)%bodyPeriod)/bodyPeriod;
      var bodyWave=Math.sin(bodyPhase*Math.PI*2);
      var bodyAmplitude=.05+energy*.063;
      ...
      var amp=1.00+energy*.63,s=r*2.49;
      var breathe1=.94+Math.sin(flowClock*.48)*(.046+energy*.014);
      var breathe2=.97+Math.sin(flowClock*.61+1.8)*(.041+energy*.016);

      drawLayer(back,cx+Math.sin(flowClock*.57)*r*.145*amp,cy+Math.cos(flowClock*.47)*r*.122*amp,
        s*1.08,Math.sin(flowClock*.31)*(.15+energy*.075),1.0,breathe1,1.04/breathe1);
      drawLayer(front,cx+Math.sin(flowClock*.84+2.1)*r*.185*amp,cy+Math.cos(flowClock*.68+1.2)*r*.158*amp,
        s*.99,-.10+Math.sin(flowClock*.42+1.2)*(.20+energy*.092),1.0,breathe2,1.035/breathe2);
      drawLayer(filaments,cx+Math.sin(flowClock*.73+3.7)*r*.148*amp,cy+Math.cos(flowClock*.88+2.5)*r*.140*amp,
        s*1.02,.13+Math.sin(flowClock*.53+3.1)*(.23+energy*.092),.82,1.02,.98);
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V68_STYLE+'</style></head>')
    .replace('</body>',ORB_V68_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v68-slightly-stronger-voice-response');
