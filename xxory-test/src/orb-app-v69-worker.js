...
const ORB_V69_STYLE = String.raw`
/* TALERA ORB v69 — v68 preserved; only whole-body scale response to voice is slightly stronger. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb69Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V69_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v69')==='1')return;
    core.setAttribute('data-orb-v69','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v69-canvas';
    ...
    function frame(now){
      ...
      /* Same voice detection and internal response as v68. */
      ...
      /* V69: idle breathing unchanged; only voice-driven whole-body scale range is ~14% stronger than v68. */
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V69_STYLE+'</style></head>')
    .replace('</body>',ORB_V69_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v69-slightly-larger-voice-scale-response');
