...
const ORB_V70_STYLE = String.raw`
/* TALERA ORB v70 — v69 preserved; only visible internal voice response is increased by about 10%. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb70Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V70_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v70')==='1')return;
    core.setAttribute('data-orb-v70','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v70-canvas';
    ...
    function frame(now){
      ...
      /* Detection and energy curve remain identical to v69. */
      ...
      var idleSpeed=.21;
      /* V70: only the voice-driven internal flow gain is +10% versus v69. */
      var speed=idleSpeed*(1+energy*4.29);
      flowClock+=dt*speed;

      /* Whole-body scale response is kept exactly at v69. */
      ...
      /* V70: internal voice-driven displacement/deformation is +10% versus v69. */
      var amp=1.00+energy*.693,s=r*2.49;
      var breathe1=.94+Math.sin(flowClock*.48)*(.046+energy*.0154);
      var breathe2=.97+Math.sin(flowClock*.61+1.8)*(.041+energy*.0176);

      drawLayer(back,cx+Math.sin(flowClock*.57)*r*.145*amp,cy+Math.cos(flowClock*.47)*r*.122*amp,
        s*1.08,Math.sin(flowClock*.31)*(.15+energy*.0825),1.0,breathe1,1.04/breathe1);
      drawLayer(front,cx+Math.sin(flowClock*.84+2.1)*r*.185*amp,cy+Math.cos(flowClock*.68+1.2)*r*.158*amp,
        s*.99,-.10+Math.sin(flowClock*.42+1.2)*(.20+energy*.1012),1.0,breathe2,1.035/breathe2);
      drawLayer(filaments,cx+Math.sin(flowClock*.73+3.7)*r*.148*amp,cy+Math.cos(flowClock*.88+2.5)*r*.140*amp,
        s*1.02,.13+Math.sin(flowClock*.53+3.1)*(.23+energy*.1012),.82,1.02,.98);
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V70_STYLE+'</style></head>')
    .replace('</body>',ORB_V70_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v70-ten-percent-stronger-internal-voice-response');
