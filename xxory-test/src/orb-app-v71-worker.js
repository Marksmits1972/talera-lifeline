...
const ORB_V71_STYLE = String.raw`
/* TALERA ORB v71 — v70 preserved; visible internal voice activity is increased by about 10%. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb71Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V71_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v71')==='1')return;
    core.setAttribute('data-orb-v71','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v71-canvas';
    ...
    function frame(now){
      ...
      /* Same microphone sensitivity/energy curve as v70; attack is 10% quicker for sharper interaction. */
      var voiceTarget=clamp(Math.pow(voiceRaw,.72)*1.15,0,.82);
      if(listening&&voiceTarget<.025)voiceTarget=.025;
      var response=voiceTarget>energy?7.7:1.35;
      energy+=(voiceTarget-energy)*Math.min(1,dt*response);

      var idleSpeed=.21;
      /* V71: voice-driven internal flow gain +10% versus v70. */
      var speed=idleSpeed*(1+energy*4.719);
      flowClock+=dt*speed;

      /* Whole-body scale response remains exactly v69/v70. */
      ...
      /* V71: internal displacement/deformation +10% versus v70. */
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V71_STYLE+'</style></head>')
    .replace('</body>',ORB_V71_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v71-ten-percent-sharper-voice-activity');
