...
const ORB_V72_STYLE = String.raw`
/* TALERA ORB v72 — v71 preserved; only voice attack is slowed by about 15%. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb72Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V72_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v72')==='1')return;
    core.setAttribute('data-orb-v72','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v72-canvas';
    ...
    function frame(now){
      ...
      /* Same microphone sensitivity and strength as v71; only attack is ~15% slower. */
      var voiceTarget=clamp(Math.pow(voiceRaw,.72)*1.15,0,.82);
      if(listening&&voiceTarget<.025)voiceTarget=.025;
      var response=voiceTarget>energy?6.545:1.35;
      energy+=(voiceTarget-energy)*Math.min(1,dt*response);

      var idleSpeed=.21;
      /* Internal flow strength remains exactly v71. */
      var speed=idleSpeed*(1+energy*4.719);
      flowClock+=dt*speed;

      /* Whole-body scale response remains exactly v69/v70/v71. */
      ...
      /* Visible internal displacement/deformation remains exactly v71. */
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V72_STYLE+'</style></head>')
    .replace('</body>',ORB_V72_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v72-fifteen-percent-slower-voice-attack');
