...
const ORB_V73_STYLE = String.raw`
/* TALERA ORB v73 — v72 preserved; only voice attack is about 7% faster. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb73Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V73_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v73')==='1')return;
    core.setAttribute('data-orb-v73','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v73-canvas';
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V73_STYLE+'</style></head>')
    .replace('</body>',ORB_V73_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v73-seven-percent-faster-voice-attack');
