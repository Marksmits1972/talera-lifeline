...
const ORB_V75_STYLE = String.raw`
/* TALERA ORB v75 — v74 preserved; hard clipping edge replaced by a soft transparent feather. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb75Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V75_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v75')==='1')return;
    core.setAttribute('data-orb-v75','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v75-canvas';
    ...
    function frame(now){
      ...
      /* Same microphone sensitivity and strength as v74/v73. */
      ...
      /* Visible internal displacement/deformation remains exactly v74/v73. */
      ...
      /* V75: feather the final 8% of the edge instead of clipping to a hard circle. */
      ctx.globalCompositeOperation='destination-in';
      var edge=ctx.createRadialGradient(cx,cy,r*.90,cx,cy,r);
      edge.addColorStop(0,'rgba(255,255,255,1)');
      edge.addColorStop(.42,'rgba(255,255,255,.97)');
      edge.addColorStop(.78,'rgba(255,255,255,.48)');
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V75_STYLE+'</style></head>')
    .replace('</body>',ORB_V75_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v75-soft-feathered-edge');
