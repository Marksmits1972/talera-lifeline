...
const ORB_V76_STYLE = String.raw`
/* TALERA ORB v76 — v75 motion preserved; rear glow removed and edge transparency moved into blue material. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb76Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V76_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function makeLayer(seed,kind){
    ...
    for(var py=0;py<TEX;py++){
      ...
      for(var px=0;px<TEX;px++){
        ...
        /* V76: no separate pale rear layer or final white alpha mask.
           The blue tissue itself becomes transparent gradually and irregularly near its perimeter. */
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v76')==='1')return;
    core.setAttribute('data-orb-v76','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v76-canvas';
    ...
    function frame(now){
      ...
      /* Same microphone sensitivity and strength as v75/v74/v73. */
      ...
      /* Visible internal displacement/deformation remains exactly v75/v74/v73. */
      ...
      /* V76: no final radial destination-in mask and no CSS rear glow. */
      ctx.restore();
    }
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V76_STYLE+'</style></head>')
    .replace('</body>',ORB_V76_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v76-no-rear-glow-organic-edge');
