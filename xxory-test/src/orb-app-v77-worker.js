...
const ORB_V77_STYLE = String.raw`
/* TALERA ORB v77 — v76 motion preserved; square canvas corrected with a soft round alpha edge, rear glow remains removed. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb77Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V77_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function makeLayer(seed,kind){
    ...
    for(var py=0;py<TEX;py++){
      ...
      for(var px=0;px<TEX;px++){
        ...
        /* Keep v76's organic transparency inside each blue tissue layer. */
        var ex=(nx-.5)*2,ey=(ny-.5)*2;
        var radial=Math.sqrt(ex*ex+ey*ey);
        var edgeWarp=(fbm(wx*.58+4.9,wy*.55-3.7,seed+131)-.5)*.10;
        var organicEdge=1-smoothstep(.78,1.04,radial+edgeWarp);
        alpha*=organicEdge;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v77')==='1')return;
    core.setAttribute('data-orb-v77','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v77-canvas';
    ...
    function frame(now){
      ...
      /* Voice behavior is exactly v76/v73. */
      ...
      /* Whole-body scale response remains exactly unchanged. */
      ...
      /* V77: canvas-space alpha-only mask restores the round silhouette.
         Black RGB is deliberate: destination-in uses alpha only, so no white layer is painted. */
      ctx.globalCompositeOperation='destination-in';
      var edge=ctx.createRadialGradient(cx,cy,r*.82,cx,cy,r);
      edge.addColorStop(0,'rgba(0,0,0,1)');
      edge.addColorStop(.46,'rgba(0,0,0,.98)');
      edge.addColorStop(.74,'rgba(0,0,0,.72)');
      edge.addColorStop(.90,'rgba(0,0,0,.28)');
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V77_STYLE+'</style></head>')
    .replace('</body>',ORB_V77_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v77-soft-round-alpha-edge-no-rear-glow');
