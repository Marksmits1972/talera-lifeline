...
const ORB_V78_STYLE = String.raw`
/* TALERA ORB v78 — reliable round v74 geometry restored; only perimeter is intentionally tinted light blue. */
.core-wrap{
...
.core,.core.active,.core.listening{
  ...
  animation:taleraOrb78Body 8.8s ease-in-out infinite!important;
}
...
const ORB_V78_SCRIPT = String.raw`<script>(function(){
  var TEX=224;
  ...
  function mountCore(core){
    if(core.getAttribute('data-orb-v78')==='1')return;
    core.setAttribute('data-orb-v78','1');
    core.innerHTML='';

    var canvas=document.createElement('canvas');
    canvas.className='orb-v78-canvas';
    ...
    function frame(now){
      ...
      /* Same microphone sensitivity and voice response as v74/v73. */
      ...
      /* Whole-body scale response remains unchanged. */
      ...
      /* V78: intentionally cover the pale iOS clipping seam with a soft light-blue inner perimeter. */
      ctx.save();
...
function enhance(html){
  return html
    .replace('</head>','<style>'+ORB_V78_STYLE+'</style></head>')
    .replace('</body>',ORB_V78_SCRIPT+'</body>');
}
...
export default {
  async fetch(request,env,ctx){
    ...
    headers.set('x-talera-orb-app','organic-v78-round-soft-light-blue-perimeter');
