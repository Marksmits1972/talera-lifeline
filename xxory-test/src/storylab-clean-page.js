...
.notice{position:fixed;z-index:40;left:50%;bottom:74px;transform:translateX(-50%) translateY(18px);opacity:0;pointer-events:none;max-width:calc(100vw - 36px);padding:10px 14px;border-radius:999px;background:rgba(16,28,39,.90);color:#fff;font-size:12px;font-weight:700;transition:.2s ease;white-space:nowrap}.notice.show{opacity:1;transform:translateX(-50%) translateY(0)}.file-input{position:fixed;left:-9999px;top:0;width:1px;height:1px;opacity:0;pointer-events:none}
...
<main id="screen" class="screen" data-revision="${STORYLAB_CLEAN_PAGE_REVISION}">
  ...
  <div class="shade"></div>
  <input id="photoInput" class="file-input" type="file" accept="image/*,video/*" multiple />
  <header class="top">
    <div>
