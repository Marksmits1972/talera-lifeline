...
async function finishWorkblad(){
  ...
  var text=(state.workText||'').trim(),spoken=String(state.liveTranscript||'').trim();
  if(spoken&&(!(state.audioBlob instanceof Blob)||!state.audioBlob.size)&&!state.hasExistingAudio){state.workError='Je woorden zijn wel herkend, maar er is geen geluidsbestand. Spreek het verhaal nogmaals in voordat je het opslaat.';return renderWorkblad()}
  ...
  try{
    if(state.editingStoryId&&state.editingToken){
      ...
      if(state.audioBlob){if(!data.hasAudio||Number(data.audioSizeBytes||0)<=0)throw new Error('De server heeft de geluidsopname niet bevestigd.');await verifyStoredAudio(state.storyId,state.manageToken)}
