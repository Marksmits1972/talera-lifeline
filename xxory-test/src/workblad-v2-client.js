...
function ensure(){
  ...
  if(typeof state.voiceAttempted!=='boolean')state.voiceAttempted=false;
...
async function draftLoad(){
  ...
  try{
    ...
    if(d){
      ...
      state.audioBlob=d.audio||null;state.duration=Number(d.duration)||0;state.liveTranscript=d.transcript||'';state.voiceAttempted=Boolean(d.voiceAttempted);
...
async function draftSave(){
  ...
  var d={id:'current',title:state.workTitle||'',date:state.workDate||'',text:state.workText||'',photos:photos,audio:state.audioBlob||null,duration:Number(state.duration)||0,transcript:state.liveTranscript||'',voiceAttempted:Boolean(state.voiceAttempted),savedAt:Date.now()};
...
function voiceHtml(){if(!state.audioBlob&&!state.hasExistingAudio)return '';return '<div class="work-voice-note"><span class="work-voice-mark"></span><span class="work-voice-copy"><strong>Gesproken verhaal toegevoegd</strong><span>'+fmt(state.duration)+' · klaar voor veilige opslag</span></span></div>'}
...
function appleRecorder(){var ua=String(navigator.userAgent||'');return /iPhone|iPad|iPod/i.test(ua)||(/Macintosh/i.test(ua)&&navigator.maxTouchPoints>1)}
function recorderMime(){var list=appleRecorder()?['audio/mp4','audio/webm;codecs=opus','audio/webm']:['audio/webm;codecs=opus','audio/webm','audio/mp4'];for(var i=0;i<list.length;i++){try{if(MediaRecorder.isTypeSupported&&MediaRecorder.isTypeSupported(list[i]))return list[i]}catch(e){}}return ''}
function audioFileName(blob){var type=String(blob&&blob.type||'').toLowerCase();return type.indexOf('mp4')>=0||type.indexOf('m4a')>=0?'verhaal.m4a':type.indexOf('ogg')>=0?'verhaal.ogg':type.indexOf('mpeg')>=0?'verhaal.mp3':'verhaal.webm'}
async function voiceStart(){capture();state.voiceAttempted=true;await draftSave();state.prevAudio=state.audioBlob||null;state.prevDuration=Number(state.duration)||0;state.prevTranscript=typeof state.liveTranscript==='string'?state.liveTranscript:'';state.prevWorkText=typeof state.workText==='string'?state.workText:'';state.workTextBeforeVoice=state.workText||'';state.hasSpeech=false;state.lastSpeechAt=0;state.voice=0;state.paused=false;state.chunks=[];state.liveTranscript='';state.recognitionFinal='';state.startedAt=0;document.documentElement.style.setProperty('--voice','0');document.documentElement.style.setProperty('--awake','0');voiceLayer('Even luisteren…',true);try{state.stream=await navigator.mediaDevices.getUserMedia({audio:true});setupWorkAudio();setupWorkRecognition();startWorkRecorder();voiceListening()}catch(e){state.audioBlob=state.prevAudio;state.duration=state.prevDuration;state.liveTranscript=state.prevTranscript;state.workText=state.prevWorkText;stopMedia();state.workError='De microfoon kon niet worden geopend. Controleer de toestemming en probeer opnieuw.';renderWorkblad()}}
function voiceLayer(message,initializing){renderWorkblad();state.view=initializing?'mic-initializing':'listening';var stage=document.querySelector('.work-stage');if(stage)stage.classList.add('voice-open');var layer=document.createElement('div');layer.id='voiceLayer';layer.className='voice-layer';layer.innerHTML='<button id="voiceClose" class="voice-close" aria-label="Sluiten">×</button><div class="voice-center">'+core(false,!initializing)+'<div id="status" class="voice-status">'+esc(message)+'</div></div><div class="voice-bottom"><button id="singleControl" class="pill hidden">Klaar</button></div>';app.appendChild(layer);document.getElementById('voiceClose').onclick=voiceCancel;document.getElementById('singleControl').onclick=finishWorkSpeech}
...
function finishWorkSpeech(){syncLiveTranscript();if(!state.hasSpeech){state.workError='Ik heb nog geen verhaal gehoord. Je kunt rustig beginnen wanneer je wilt.';voiceCancel();return}state.view='finishing';var r=state.recorder;if(r&&r.state!=='inactive'){try{if(typeof r.requestData==='function')r.requestData()}catch(e){}setTimeout(function(){try{if(r.state!=='inactive')r.stop()}catch(e){}},120)}else{stopMedia();draftSave().finally(renderWorkblad)}}
function startWorkRecorder(){var mime=recorderMime();state.chunks=[];var r=new MediaRecorder(state.stream,mime?{mimeType:mime}:undefined);state.recorder=r;r.ondataavailable=function(e){if(e.data&&e.data.size)state.chunks.push(e.data)};r.onerror=function(){state.workError='De geluidsopname is onderbroken. Probeer het inspreken nog een keer.'};r.onstop=function(){var type=r.mimeType||mime||(appleRecorder()?'audio/mp4':'audio/webm');var b=new Blob(state.chunks,{type:type});syncLiveTranscript();if(state.hasSpeech&&b.size){state.audioBlob=b;state.duration=state.startedAt?Math.max(0,(Date.now()-state.startedAt)/1000):0;state.workError=''}else if(state.hasSpeech){state.audioBlob=state.prevAudio||null;state.duration=state.prevDuration||0;state.workError='Je woorden zijn wel herkend, maar de geluidsopname is niet vastgelegd. Probeer Vertel iets nogmaals.'}stopMedia();state.view='workblad';draftSave().finally(renderWorkblad)};if(appleRecorder())r.start();else r.start(500)}
...
async function verifyStoredAudio(storyId,token){var path='/api/integration/stories/'+encodeURIComponent(storyId)+'/audio';var head=await fetch(path,{method:'HEAD',headers:{'authorization':'Bearer '+token},cache:'no-store'});var bytes=Number(head.headers.get('content-length')||0);if(!head.ok||bytes<=0)throw new Error('De geluidsopname staat nog niet veilig op de server. Probeer het opnieuw.');var detailRes=await fetch('/api/integration/stories/'+encodeURIComponent(storyId),{headers:{'authorization':'Bearer '+token},cache:'no-store'});var detail={};try{detail=await detailRes.json()}catch(e){}if(!detailRes.ok||!detail.hasAudio||Number(detail.audioSizeBytes||0)<=0)throw new Error('De herinnering is nog niet aan de geluidsopname gekoppeld.');return detail}
async function saveEditedAudio(storyId,token){if(!(state.audioBlob instanceof Blob)||!state.audioBlob.size)return;var fd=new FormData();fd.append('audio',state.audioBlob,audioFileName(state.audioBlob));fd.append('durationSeconds',String(Number(state.duration)||0));var res=await fetch('/api/integration/stories/'+encodeURIComponent(storyId)+'/audio',{method:'PUT',headers:{'authorization':'Bearer '+token},body:fd,cache:'no-store'});var data={};try{data=await res.json()}catch(e){}if(!res.ok||!data.hasAudio)throw new Error(data.error||'De geluidsopname kon niet worden opgeslagen.');await verifyStoredAudio(storyId,token);state.hasExistingAudio=true}
async function finishWorkblad(){
  capture();state.workError='';await draftSave();
  var text=(state.workText||'').trim(),date=(state.workDate||'').trim();
  if(!date){state.workError='Vul eerst in wanneer deze herinnering was.';return renderWorkblad()}
  if(state.voiceAttempted&&(!(state.audioBlob instanceof Blob)||!state.audioBlob.size)&&!state.hasExistingAudio){state.workError='Je hebt Vertel gebruikt, maar er is geen geluidsbestand vastgelegd. Spreek het verhaal nogmaals in voordat je het opslaat.';return renderWorkblad()}
  if(!state.audioBlob&&!state.hasExistingAudio&&!text){state.workError='Voeg eerst iets toe: een gesproken stukje of wat tekst.';return renderWorkblad()}
  state.view='saving';var ov=document.createElement('div');ov.className='work-saving';ov.innerHTML='<div class="work-saving-card"><div class="processing"></div><h2>Je blad wordt op zijn plek gezet</h2><div class="muted">Je stem wordt eerst veilig opgeslagen. Daarna kom je terug bij deze herinnering op je tijdlijn.</div></div>';document.body.appendChild(ov);
  try{
    if(state.editingStoryId&&state.editingToken){
      await saveEditedAudio(state.editingStoryId,state.editingToken);
      ...
      var fd=new FormData();if(state.audioBlob)fd.append('audio',state.audioBlob,audioFileName(state.audioBlob));if(text)fd.append('storyText',text);if(state.liveTranscript)fd.append('liveTranscript',state.liveTranscript);
      ...
      if(first)fd.append('startPhoto',first,first.name||'herinnering.jpg');fd.append('sourceMode','workblad');fd.append('durationSeconds',String(state.duration||0));fd.append('voiceAttempted',state.voiceAttempted?'1':'0');fd.append('eventTime',date);fd.append('title',(state.workTitle||'').trim());fd.append('sessionId',sessionId);var name=localStorage.getItem('xxory-display-name');if(name)fd.append('displayName',name);
      var res=await fetch('/api/integration/stories',{method:'POST',body:fd,cache:'no-store'}),data={};try{data=await res.json()}catch(e){}if(!res.ok)throw new Error(data.error||'Opslaan mislukt');state.storyId=data.storyId;state.manageToken=data.manageToken;state.proposal=data.proposal||{};
      if(state.voiceAttempted){if(!data.hasAudio||Number(data.audioSizeBytes||0)<=0)throw new Error('De server heeft de geluidsopname niet bevestigd.');await verifyStoredAudio(state.storyId,state.manageToken)}
...
async function loadExisting(ctx){
  ...
  state.workTitle=data.title||'';state.workDate=data.eventTime||'';state.workText=data.textContent||'';state.duration=Number(data.durationSeconds)||0;state.audioBlob=null;state.hasExistingAudio=Boolean(data.hasAudio);state.voiceAttempted=Boolean(data.hasAudio);state.liveTranscript='';
